...
const { Transaction } = require('starkbank');
...
beforeAll(async () => {
    ...
    //const response = await axios.post(`${uri}/api/client/`, { phone: phoneForTest })
    /*
    let category;
    ...
    for (const element of dataTest.products) {
        ...
        for (const item of element.item) {
            try {
                ...
                // Verifica a resposta da requisição (opcional)
                //console.log(`Item ${item.type} criado com sucesso. Status: ${res.status}`);
    ...
    */
    ...
    it('Etapa=6 e Cadastro Pedido', async () => {
     ...
     it('Etapa=8 e Tipo de Pagamento', async () => {
         const msg = { from: phoneForTest, body: '3' };
         const response = await dataMenu(msg);
         console.log(response);
